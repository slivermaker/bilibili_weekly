...
def select_data(json_fpath):
    ...
    _df[["tname", "title", "desc", "rcmd_reason"]] = _base_info[
        ["tname", "title", "desc", "rcmd_reason"]
    ]
    # 获取up主的信息
...
def clean_data(dataframe):
    ...
    for i in range(desc_none.shape[0]):
        dataframe.loc[desc_none.index[i], "desc"] = dataframe.loc[
            desc_none.index[i], "title"
        ]
    # 处理推荐理由为空的数据，使用该视频的标题进行填充
    rcmd_reason_none = dataframe.loc[
        (dataframe["rcmd_reason"] == "") | (dataframe["rcmd_reason"] == "-")
    ]
    for i in range(rcmd_reason_none.shape[0]):
        dataframe.loc[rcmd_reason_none.index[i], "rcmd_reason"] = dataframe.loc[
            rcmd_reason_none.index[i], "title"
        ]
    # 对长文本进行处理
    dataframe["title"] = dataframe["title"].map(lambda x: x.replace(",", " "))
    dataframe["desc"] = dataframe["desc"].map(lambda x: x.replace(",", " "))
    dataframe["rcmd_reason"] = dataframe["rcmd_reason"].map(
        lambda x: x.replace(",", " ")
    )
    dataframe["title"] = dataframe["title"].map(lambda x: x.replace(";", " "))
    dataframe["desc"] = dataframe["desc"].map(lambda x: x.replace(";", " "))
